...
def run_ctas(schema, table, select_sql, primary_key=None):
    ...
    try:
        ...
        cur.execute(f"""
            SELECT COUNT(*) - COUNT(DISTINCT sessionId) AS dup_count
            FROM {schema}.temp_{table}
        """)
        dup_pk = (cur.fetchone() or [0])[0]
        if dup_pk and int(dup_pk) > 0:
            raise Exception(f"[FAIL] Duplicates in temp table on sessionId: {dup_pk}")
